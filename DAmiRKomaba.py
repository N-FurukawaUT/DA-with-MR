...
Comment
-------
DA with minority reserveのプログラムを組んでみました。
Pandasパッケージが追加されていれば使えます。
...
import numpy as np
class DAmiRKomaba():
    def __init__(self, submitcsv, acceptcsv,n, isDisplay=False, filename='submitmirResultKomaba'):
        """
        Parameters
        ----------
        submitcsv : str
...
#                acceptdictValues=list(acceptdict.values()) #acceptdictの値をlistにまとめる。値は各受け入れ者の仮受け入れ応募者。
            ...
            if cnt==len(sl):
                ...
                np.save(f"MultiThread/sim_fill_MR{self.n}.npy", acceptdictMinorityReserve)
                dataMinority.to_csv(f"MultiThread/minority_acceptance_MR{self.n}.csv", header=0, index=0, encoding='utf-8-sig')
                dataMajority=pd.DataFrame(dataframeMajority)
                dataMajority.to_csv(f"MultiThread/majority_acceptance_MR{self.n}.csv", header=0, index=0, encoding='utf-8-sig')
                ...
                for i in subd.keys(): #iは大学名
                    acceptance, minorityacceptanceDict, minorityacceptanceAll, majorityacceptance=DAmiRKomaba.accept(self, acceptdictDA[i], aDic[i], subd[i],MRDictDA[i], secondCapacity[i], karuidic, i, sl) #応募者が出願した受け入れ者の仮受け入れ応募者を決定する。
    ...
    def accept(self, acceptance, aPref, submitter,MRDict, capacity, karuidict, accepter,sl):
        for i in submitter:
            acceptance.append(i)
        aPrefminus=aPref[:]
        acceptancePref=aPref[:]
        for i in acceptance:
            aPrefminus.remove(i)
        for i in aPrefminus:
            acceptancePref.remove(i) #受け入れ者の選好順序リスト(申込者のみ)
        minorityDict=dict()
        minorityListAll=[]
        ...
        acmajority=list(set(acceptancePref)&set(majoritylist)) #acceptanceの中のマジョリティリスト
        ...
        if len(majorityacceptance)>capacity-len(minorityacceptanceAll): #もしマジョリティの数が残りの枠より多ければ
            majorityacceptance[capacity-len(minorityacceptanceAll):len(majorityacceptance)]=[] #オーバーした分をリストから削除
...
if __name__=="__main__": #importしたときに、このif文以下を実行しないようにするための条件
    ...
    #DAmiR('submitmir.csv','acceptmir.csv', isDisplay=False) #関数の実行例。デフォルトのinputでは学生を応募者に、大学を受け入れ者側として実行
    DAmiRKomaba('MultiThread\created_submit_komaba0.csv','MultiThread\created_accept_komaba0.csv', 0, filename="submitmirResultKomaba0")
